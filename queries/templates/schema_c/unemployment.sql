-- unemployment.sql (Schema C)
-- Returns mergeable unemployment partials using JSON employment.has_job.
...
extracted AS (
  SELECT
    ...
    CASE
      WHEN (b.profile_json #>> '{employment,has_job}') IN ('true','false')
        THEN (b.profile_json #>> '{employment,has_job}')::BOOLEAN
      ELSE NULL
    END AS has_job,
    (b.profile_json #>> '{employment,has_job}') AS has_job_raw,
...
labeled AS (
  SELECT
    ...
    (e.has_job_raw IS NOT NULL AND e.has_job_raw <> '' AND e.has_job IS NULL) AS invalid_has_job,
...
valid AS (
  SELECT
    l.*,
    NOT (
      l.missing_required OR l.missing_verification OR l.invalid_has_job OR
    ...
    CASE
      WHEN l.missing_required OR l.invalid_has_job THEN 'missing_required'
...
agg AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE is_valid AND has_job IS TRUE)::BIGINT AS employed_count,
    COUNT(*) FILTER (WHERE is_valid AND has_job IS FALSE)::BIGINT AS not_employed_count
  FROM valid
)
SELECT * FROM agg;