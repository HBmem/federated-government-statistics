-- population.sql (Schema B)
-- Returns population count for residents valid in timeframe.
-- Date parsing is best-effort and treats unparseable dates as invalid_date drops.
...
labeled AS (
  SELECT
    ...
    -- If moved_in_str exists but couldn't parse => invalid_date
...
valid AS (
  SELECT
    l.*,
    NOT (
      l.missing_required OR
      l.missing_verification OR
