-- unemployment.sql (Schema B)
-- Returns mergeable unemployment partials.
-- Uses has_job_flag where 'Y' means employed.
...
labeled AS (
  SELECT
    ...
    (p.has_job_flag IS NULL OR p.has_job_flag = '') AS missing_job_flag
  FROM parsed p
),
valid AS (
  SELECT
    l.*,
    NOT (
      l.missing_required OR l.missing_verification OR l.missing_job_flag OR
    ...
    CASE
      WHEN l.missing_required OR l.missing_job_flag THEN 'missing_required'
...
agg AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE is_valid AND has_job_flag = 'Y')::BIGINT AS employed_count,
    COUNT(*) FILTER (WHERE is_valid AND has_job_flag <> 'Y')::BIGINT AS not_employed_count
  FROM valid
)
SELECT * FROM agg;